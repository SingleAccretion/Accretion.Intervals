...
namespace Accretion.Intervals.Tests
{
    ...
    {
        public static Arbitrary<int> Int32s() => Arb.From(
            from result in Gen.Frequency(
        ...
        public static Arbitrary<float> Singles() => Arb.From(
            from value in Arb.Default.DoNotSizeInt32().Generator.Select(x => BitConverter.Int32BitsToSingle(x.Item))
        ...
        public static Arbitrary<double> Doubles() => Arb.From(
            from value in Arb.Default.DoNotSizeInt64().Generator.Select(x => BitConverter.Int64BitsToDouble(x.Item))
        ...
        public static Arbitrary<BoundaryType> BoundaryTypes() => Arb.From(Arb.Generate<bool>().Select(x => x ? BoundaryType.Open : BoundaryType.Closed));

        public static Arbitrary<ValueClass> ValueClasses() => Arb.From(Arb.Generate<int?>().Select(x => (ValueClass)x));
        ...
        public static Arbitrary<FormatString> Formats() => Arb.From(Gen.Elements("G", "N", "P", "F", "C").Select(x => new FormatString(x)));
    }
}
