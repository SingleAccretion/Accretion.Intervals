...
namespace Accretion.Intervals
{
    public static class IntervalsTests
    {
        ...
        {            
            Parallel.For(0, RandomIntervals.Count - 1, i =>
            {
                var interval = RandomIntervals[i];
                var nextInterval = RandomIntervals[i + 1];

                if (!TestContains(interval) || !TestUnion(nextInterval, interval) || !TestIntersect(nextInterval, interval) || !TestSymmetricDifference(nextInterval, interval))
                {
                    throw new Exception();
                }
        ...
        public static bool TestSymmetricDifference(Interval<int> first, Interval<int> second)
        {
            var testResult = true;
